...
var phantomjs = module.exports = new EventEmitter2({wildcard: true});
...
phantomjs.spawn = function(pageUrl, options) {
  ...
  (function loopy() {
    ...
    var done = lines.slice(n).some(function(line) {
      ...
      if (eventName === 'private') {
        ...
        phantomjs.emit.apply(phantomjs, args);
      }
  ...
  // Build an array of optional PhantomJS --args
  var args = [];
  Object.keys(options.options.phantomjs).forEach(function(key) {
    var optionValue = options.options.phantomjs[key];

    if (optionValue === true)       optionValue = 'yes';
    else if (optionValue === false) optionValue = 'no';

    args.push('--' + key + '=' + optionValue);
  });
  ...
  return grunt.util.spawn({
    ...
    args: args.concat([
      ...
      JSON.stringify(bootstrapOptions)
    ])
