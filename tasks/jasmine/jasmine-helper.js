...
GruntReporter.prototype = {
  ...
  reportRunnerStarting : function(runner) {
    this.log('>> Jasmine Starting <<');
  },
  reportSuiteStarting : function(suite) {
    this.log("MONKEYS!");
  },
  reportSpecStarting : function(spec) {
    this.log('>> Suite ' + spec.suite.id + '-' + spec.suite.description);
  ...
  reportSuiteResults : function(suite) {
    ...
    this.log(">>>> " + suite.description + " " + status);
  },
  ...
  reportSpecResults : function( spec ){
    ...
    for (var i = 0; i < items.length; i++) {
      ...
      if (item.type === 'log') {
        ...
        this.log(">>>> Spec Failed : " + item.message);
        if (item.trace.stack) {
...
(function() {
  ...
  JUnitXmlReporter.prototype = {
    ...
    writeFile: function(filename, text) {
      sendMessage('junit',filename,text);
    },
